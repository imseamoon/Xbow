...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="dark" suppressHydrationWarning>
      <body className={`${geistSans.variable} ${geistMono.variable} antialiased selection:bg-pink-500/30 font-sans`}>
        <div className="flex min-h-screen bg-[#0A0A0B]">
          {/* ── Sidebar ───────────────────────────────────── */}
          <aside className="fixed inset-y-0 left-0 z-50 w-64 flex-col border-r border-white/10 bg-[#0A0A0B] hidden md:flex">
            <div className="p-6">
              <div className="flex items-center gap-3">
                <div className="size-8 rounded-lg bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center shadow-sm">
                  <Shield size={18} className="text-white" />
                </div>
                <span className="font-bold text-slate-100 tracking-wide">RedSentinel</span>
              </div>
            </div>
            
            <div className="flex-1 overflow-y-auto py-4 px-4">
              <div className="text-xs font-semibold text-slate-500 mb-4 px-2 uppercase tracking-widest">Menu</div>
          ...
          <main className="flex-1 md:pl-64 flex flex-col min-h-screen">
            {/* Top Bar */}
            <header className="sticky top-0 z-40 h-14 border-b border-white/10 bg-[#0A0A0B]/80 backdrop-blur-md px-6 flex items-center justify-between">
              ...
              {/* Unused 'System Active' badge removed to clean up UI */}
            </header>
