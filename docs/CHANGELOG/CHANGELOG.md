...
The labeler checked `on\w+=` (event_handler) _before_ checking for full HTML tags, template patterns, and DOM sinks. This caused `<img onerror=…>`, `<svg onload=…>`, `document.body.innerHTML=…`, and `{{template}}` payloads to all be classified as `event_handler`, inflating that class to 78% of the dataset.
...
## 10. XSS Scanner Bottlenecks (Google XSS Game Levels 3-6)

**Problem:** The Xbow scanner successfully detected Reflected and Stored XSS (Levels 1-2) but completely failed on advanced client-side vulnerabilities like DOM-based XSS via location hash, Javascript-in-attribute execution, and Javascript URIs (Levels 3-6).

**Root Causes (5 compounding gaps):**

1. **Fragment Injection Blockage:** The scanner architecture was query-parameter-centric. `scan.processor.ts` ignored `__fragment__` params because they had no server-side reflection, destroying the entire DOM XSS pipeline. `http_sender.py` also mistakenly sent HTTP requests for fragments.
2. **Missing JS-in-event-handler Context:** Though `html_parser.py` identified `onload="..."` as `js_string`, the payload bank selector didn't map `js_string` to the `event_handler` context.
3. **JS-string-breakout Payloads Missing:** Fast breakout payloads like `');alert(1)//` were practically non-existent in the dataset.
4. **Javascript URI Verification Blockage:** `browser_verifier.py` navigated to URLs injecting `<a href="javascript:alert(1)">` but failed to actually _click_ the links unless the payload text explicitly contained the word "javascript", missing edge-case mutations.
5. **Data URI & Protocol-Relative Missing:** No payloads like `data:text/javascript,alert(1)` existed in the dataset for script source hijacking (`<script src=...`).

**Solutions:**

- **Pipeline Bypass for Fragments:** Updated `scan.processor.ts` to inject a synthetic `html_body` context for `__fragment__` params with 0 server-reflection. Modified fuzzer `app.py` and `http_sender.py` to bypass the HTTP sending phase entirely for fragments, routing them directly to the `browser_verifier.py` (which navigates to `url#payload`).
- **Selector Mappings:** Updated `selector.py` to map `url` → `js_uri`, `js_string` → `event_handler`, and added a new `script_src` context.
- **Payload Bank Expansion:** Directly added targeted `js_uri` and `script_injection` breakout payloads (e.g. `');alert(1)//`, `data:...,alert(1)`) into the 24k+ DB CSV.
- **Link Click Verification:** Modified `browser_verifier.py` to aggressively query `a[href^="javascript:"]` and execute clicks automatically.

**Affected Files:**

- `core/src/queue/scan.processor.ts`
- `modules/fuzzer-module/app.py`
- `modules/fuzzer-module/http_sender.py`
- `modules/fuzzer-module/browser_verifier.py`
- `modules/payload-gen-module/selector.py`
- `dataset/processed/payloads_labeled.csv` and `dataset/splits/train.csv`
...
| 8   | datetime column cross-DB issue                        | Persistence        | Low      | ✅ Fixed |
| 9   | Dataset class imbalance + mislabeling (4 root causes) | ML/Data            | Critical | ✅ Fixed |
| 10  | XSS Scanner Bottlenecks (Google XSS Game Levels 3-6)  | Detection Pipeline | High     | ✅ Fixed |
>>>>>>>> Stashed changes:docs/CHANGELOG/CHANGELOG.md
